...
#GRAFICO 4: Variación Diaria de Humedad, Presión y Temperatura-----------
st.markdown('<h3 style="color:navy;">5. Variación Diaria de Humedad, Presión y Temperatura</h3>', unsafe_allow_html=True)

# Filtrar el DataFrame según el rango de fechas seleccionado
evolution_data = df2.loc[start_date:end_date]

# Calcular la variación diaria para Humedad, Presión y Temperatura
evolution_data['Variacion Humedad'] = evolution_data['Humedad (%)'].diff()
evolution_data['Variacion Presion'] = evolution_data['Presion (Pa)'].diff()
evolution_data['Variacion Temperatura'] = evolution_data['Temperatura (C)'].diff()
...
sns.lineplot(x='Fecha', y='Variacion Humedad', data=evolution_data, label='Variación Humedad', ax=ax_variation)

# Graficar la variación diaria de la presión
sns.lineplot(x='Fecha', y='Variacion Presion', data=evolution_data, label='Variación Presión', ax=ax_variation)

# Graficar la variación diaria de la temperatura
sns.lineplot(x='Fecha', y='Variacion Temperatura', data=evolution_data, label='Variación Temperatura', ax=ax_variation)

# Configurar las etiquetas y el título
ax_variation.set(xlabel='Fecha', ylabel='Variación Diaria', title='Variación Diaria de Humedad, Presión y Temperatura')
ax_variation.legend()
